...
//should be 159 and closest is 610
const path3 = "R75,D30,R83,U83,L12,D49,R71,U7,L72";
const path4 = "U62,R66,U55,R34,D71,R55,D58,R83";

//135 and closest 410
const path5 = "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51";
const path6 = "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7";
...
test("find closes distance of intersection flow", () => {
  const closest1 = findClosestIntersection(getPathIntersections(path3, path4));
  const closest2 = findClosestIntersection(getPathIntersections(path5, path6));
  const closest3 = findClosestIntersection(getPathIntersections(path1, path2));
  const closest4 = findClosestIntersection(getPathIntersections(path7, path8));
  console.log(closest4);
  expect(closest3).toEqual(6);
  expect(closest1).toEqual(159);
  expect(closest2).toEqual(135);
});
test("checkShortestSteps", () => {
  expect(checkShortestSteps(path3, path4)).toEqual(610);
  expect(checkShortestSteps(path5, path6)).toEqual(410);
});

test("getPathIntersections", () => {
  expect(getPathIntersections(path1, path2)).toEqual(
...
test("findClosestIntersection", () => {
  const intersections = [
    [3, 3],
    [5, 6]
  ];
  expect(findClosestIntersection(intersections)).toEqual(6);
});

test("createPathPointsMatrix", () => {
  const parsed = parsePath(path2);

  const totalSteps = parsed.reduce((prev, curr) => {
    return prev + curr.distance;
  }, 0);

  const matrix = createPathPointsMatrix(path1);

  expect(matrix.length).toEqual(totalSteps);
});

test("parsePath", () => {
  expect(parsePath(path1)).toStrictEqual([
...
test("getNextPointCoordinates", () => {
  expect(
    getNextPointCoordinates({ direction: "R", distance: 8 })
  ).toStrictEqual([8, 0]);

  expect(
    getNextPointCoordinates({ direction: "U", distance: 5 }, [8, 0])
  ).toStrictEqual([8, 5]);

  expect(
    getNextPointCoordinates({ direction: "L", distance: 5 }, [8, 5])
  ).toStrictEqual([3, 5]);

  expect(
    getNextPointCoordinates({ direction: "D", distance: 3 }, [3, 5])
  ).toStrictEqual([3, 2]);
});

test("getCoordinatesBetweenPoints", () => {
  const testHorizontal = getCoordinatesBetweenPoints([8, 12], [5, 12]);
  const testVertical = getCoordinatesBetweenPoints([8, 1], [8, -2]);

  expect(testHorizontal).toEqual(
    expect.arrayContaining([
      [8, 12],
      [7, 12],
      [6, 12]
